...
impl AudioManager {
    pub fn new() -> Result<Self, &'static str> {
        let (stream, stream_handle) =
            OutputStream::try_default().map_err(|_| "Failed to initialize audio output")?;
...
impl Default for AudioManager {
    fn default() -> Self {
        Self::new().expect("native audio output unavailable")
    }
}
...
impl Default for AudioManager {
    fn default() -> Self {
        Self::new().expect("fallback audio manager construction cannot fail")
    }
}
...
impl ImageDecoder {
    ...
    pub fn decode(&self, data: &[u8]) -> Result<DecodedImage, &'static str> {
        let img = image::load_from_memory(data).map_err(|_| "Failed to decode image")?;
